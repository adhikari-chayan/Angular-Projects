...
export class SecurityService {
  ...
  login(entity: AppUser): Observable<AppUserAuth>{
   //Delete userId property for posting to server
   delete entity.userId;

   return this.http.post<AppUserAuth>(this.apiUrl + "login", entity, httpOptions)
                   .pipe(
                      tap(resp => {
                      ...
                      catchError(
                        this.handleError<AppUserAuth>('login', 'Invalid user name/password', new AppUserAuth())
                      )
                   );
  ...
  hasClaim(claimType: any, claimValue?: any) : boolean{
    ...
    //See if an array of valus was passed in
    ...
    return (error: any): Observable<T> => {
      ...
      switch (error.status) {
        ...
        case 0:
          ...
          break;     
        case 403:
          this.msgService.addExceptionMessage("Error 403: Unable to authenticate the user");
          break;
        default:
